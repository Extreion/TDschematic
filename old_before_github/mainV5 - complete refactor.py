...
def replace_av_numbers(handle):
    ...
    if use_av_numbers:
        ...
        if produce_excel_schedule:
            ...
            excel_rows.append([room_info.floor, room_info.room, cable_type, room_info.av_number,
                               room_info.cable_number, cable_purpose, cable_from, cable_to])
        ...
        if equipment != "":
            ...
            for ite in range(int(quantity)):
                ...
                if not room_info.labels_created:
                    create_room_label(room_info.floor, room_info.x + 4, room_info.y - 4)
                    create_room_label(room_info.room, room_info.x + 44, room_info.y - 4)
                    page.y_pointer -= 12
                    room_info.height += 12
